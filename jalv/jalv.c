...
///
	...
	if (jalv->externalui) {
		...
		const LV2_Feature external_lv_feature = {
			LV2_EXTERNAL_UI_URI, parent
		};
		const LV2_Feature external_kx_feature = {
			LV2_EXTERNAL_UI_KX__Host, parent
		};
...
//	Jalv jalv;
	...
	if (jalv.opts.preferred_jack_client_name) {
		printf("preferred jack client name: %s\n", jalv.opts.preferred_jack_client_name);
	}
	...
	if (jalv.opts.preferred_jack_client_name)
	{
		name_str = jalv.opts.preferred_jack_client_name;
	}
	else
	{
